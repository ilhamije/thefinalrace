...
class SparseShiftConv2d(BaseModule):
    """Shift module based on Dacheng Li et al.'s paper:
    'Shift: A Zero FLOP, Zero Parameter Alternative to Spatial Convolutions'.

    Applies fixed directional shift to individual channels.
    """

    def __init__(self, in_channels, kernel_size=3, use_1x1_after_shift=True, init_cfg=None):
        super().__init__(init_cfg)
        self.in_channels = in_channels
        self.kernel_size = kernel_size
        self.use_1x1 = use_1x1_after_shift

        # Generate shift offsets (dy, dx) per channel
        self.shifts = self._generate_shifts(in_channels, kernel_size)

        if self.use_1x1:
            self.pointwise = nn.Conv2d(in_channels, in_channels, kernel_size=1)

    def _generate_shifts(self, channels, k):
        """Evenly assign shift directions across channels"""
        directions = [(i - k // 2, j - k // 2)
                      for i in range(k) for j in range(k)]
        total_dirs = len(directions)
        per_dir = channels // total_dirs
        extra = channels % total_dirs

        shifts = []
        for i, (dy, dx) in enumerate(directions):
            count = per_dir + (1 if i < extra else 0)
            shifts.extend([(dy, dx)] * count)
        return shifts
    ...
    def forward(self, x):
        ...
        for c in range(C):
            dy, dx = self.shifts[c]
            # Efficient shift using torch.roll
            shifted = torch.roll(x[:, c], shifts=(dy, dx), dims=(1, 2))
            out[:, c] = shifted
...
class MSCABlock(BaseModule):
    """MSCABlock with Sparse Shift Convolution (Dacheng Li's paper version).

    Args:
        channels (int): Number of channels.
        mlp_ratio (float): Ratio to expand hidden dim in MLP.
        drop (float): Dropout rate in MLP.
        drop_path (float): Drop path rate.
        act_cfg (dict): Activation config.
        norm_cfg (dict): Norm config.
        use_1x1_after_shift (bool): Whether to apply 1x1 conv after shift.
    """
