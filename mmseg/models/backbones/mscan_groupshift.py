...
# class GroupShiftConv(nn.Module):
#     """Grouped Channel-wise Shift Convolution."""

#     def __init__(self, channels, shift_size=1, groups=4):
#         super(GroupShiftConv, self).__init__()
#         self.channels = channels
#         self.shift_size = shift_size
#         self.groups = groups
#         self.pw_conv = nn.Conv2d(
#             channels, channels, kernel_size=1)  # Pointwise conv
...
#         if split_size == 0:
#             x_shift = torch.roll(x, shifts=self.shift_size, dims=3)
#         else:
#             x_shift = []
#             for i in range(self.groups):
#                 shift_dim = (2, 3) if i == self.groups - \
#                     1 else (2 if i % 2 == 0 else 3)
#                 x_shift.append(torch.roll(x[:, i * split_size:(i + 1) * split_size, :, :],
#                                           shifts=self.shift_size, dims=shift_dim))
#             x_shift = torch.cat(x_shift, dim=1)

#         return self.pw_conv(x_shift)


# class MSCABlock(BaseModule):
#     """Basic Multi-Scale Convolutional Attention Block with Grouped Shift Convolution."""
...
#         # Replacing standard attention with GroupShiftConv
#         self.attn = GroupShiftConv(channels)
