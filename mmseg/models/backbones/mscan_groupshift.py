...
class GroupedShiftConv2d(BaseModule):
    ...
    def create_shift_offsets(self):
        ...
        for g in range(self.num_groups):
            dx = (g % self.kernel_size - k) * self.dilation
            dy = (g // self.kernel_size - k) * self.dilation
    ...
    def shift(self, x):
        ...
        for g, (dx, dy) in enumerate(self.shift_offsets):
            c_start = g * self.group_channels
            c_end = (g + 1) * self.group_channels
            x_group = x[:, c_start:c_end]

            # Shift operation (roll)
            x_group = torch.roll(x_group, shifts=(dy, dx), dims=(2, 3))
            x_shifted[:, c_start:c_end] = x_group
