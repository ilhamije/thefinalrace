_base_ = [
    ...
    '../_base_/datasets/rescuenet_512x512.py'  # ✅ Retains ColorJitter
]

checkpoint_file = 'https://download.openmmlab.com/mmsegmentation/v0.5/pretrain/segnext/mscan_t_20230227-119e8c9f.pth'
crop_size = (512, 512)

# ✅ GroupNorm in decode head
ham_norm_cfg = dict(type='GN', num_groups=32, requires_grad=True, eps=1e-5)
...
model = dict(
    ...
    backbone=dict(
        ...
        drop_path_rate=0.1,
        depths=[3, 3, 5, 2],
        use_1x1_after_shift=True,  # ✅ Include 1×1 conv
        act_cfg=dict(type='GELU'),  # ✅ GELU activation
        norm_cfg=dict(type='GN', num_groups=32,
                      requires_grad=True, eps=1e-5)  # ✅ GroupNorm
    ),
    decode_head=dict(
        type='LightHamHead',
